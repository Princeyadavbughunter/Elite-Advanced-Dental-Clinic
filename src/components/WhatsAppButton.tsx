...
export default function WhatsAppButton({ branch }: WhatsAppButtonProps) {
  const primaryPhone = branch ? branch.contact.phones[0] : "9922018080";
  const branchName = branch ? branch.name : "Gurunanak Colony";

  const phoneNumber = primaryPhone.replace(/\+/g, '').replace(/\s/g, '').replace(/-/g, '');
  const message = `Hello! I would like to book an implant consultation at Elite Advanced Dental Implants and Aligners Clinic (${branchName}).`;
